...
Dphy=[4:1:55]*10^(-3); % m distans cavity
tp=2*10^(-2); % коэффициент пропуская первого зеркала (в отн.единицах)
tout=2*10^(-2); % коэффициент пропуская 2-ого зеркала
alfa=1*10^(-2); % Измеренные внутренние потери резонатора на длине волны резонирующего сигнала
nair=1;
r=120*10^(-3) %  радиус кривизны зеркал в метрах
lambda = 870*10^(-9); % метра
...
Tr=(nair*2*(Dphy-L)+ns*2*L)/c0; % время полного обхода резонатора туда и обратно в секундах
...
Finesse=2*pi/(tp+tout+2*alfa); % Finesse
...
ax1 = nexttile;
plot(d1,U)
title(ax1,'stability region of the cavity')
ylabel(ax1,'U')
xlabel(ax1,'d (mm)')
grid on
